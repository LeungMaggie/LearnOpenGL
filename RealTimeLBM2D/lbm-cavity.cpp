...
double rho[NX+1][NY+1],u[NX+1][NY+1][2],u0[NX+1][NY+1][2],rhoLast[NX+1][NY+1],
	f[NX+1][NY+1][Q],F[NX+1][NY+1][Q], dF[NX+1][NY+1][Q]; 
double c,Re,dx,dy,Lx,Ly,dt,rho0,P0,tau_f,niu,error,alpha;
...
void init()
{
	...
	alpha = 1.0;
	tau_f=3.0*niu+0.5;
	//tau_f=2.0/(3.0-2.0*3.0*niu);
	//tau_f=(2.0*tau*alpha-alpha+2.0)/(2.0*tau*alpha-2.0*tau+3.0-alpha);
...
{ 
	...
	for(j=1;j<NY;j++)
	{
		...
		u[i][j][1]=0;
		for(k=0;k<Q;k++) {
            //f[i][j][k] = F[i][j][k]-(1.0-alpha)*(f[i][j][k]-feq(k,rhoLast[i][j],u0[i][j]));
			f[i][j][k]=F[i][j][k];
			rho[i][j]+=f[i][j][k];
